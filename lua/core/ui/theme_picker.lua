-- Interactive theme picker (floating window).
--
-- Insert-mode prompt with a virtual cursor, debounced live preview, and mouse
-- support. Owns no plugin of its own — it drives core.theme, which is the
-- single source of truth for the registry, persistence, and application.
--
-- Why this is a plain module and not a lazy spec: it used to be declared as a
-- fake plugin with `dir = stdpath("config")`. lua/plugins/images.lua used the
-- same dir, and lazy.nvim resolves local specs by dir, so the two unrelated
-- fragments were merged into one plugin. As a module it is still lazy — nothing
-- here runs until the first require — without the fake-plugin machinery.
local M = {}
...
-- nvim_buf_add_highlight() is deprecated since Neovim 0.11 (this config
-- requires 0.11+, so the replacement is always available). vim.hl.range takes
-- {line, col} tuples and an inclusive/exclusive end; wrap it once here so the
-- render loop keeps reading like the old single-line call.
---@param line integer 0-indexed buffer line
---@param col_start integer 0-indexed byte column
---@param col_end integer 0-indexed byte column, or -1 for end-of-line
local function add_highlight(buf, group, line, col_start, col_end)
  vim.hl.range(buf, ns, group, { line, col_start }, { line, col_end == -1 and 2 ^ 31 or col_end })
end
...
function M.open()
  ...
  local function render()
    ...
    local ok, err = pcall(function()
      ...
      -- Separator
      add_highlight(buf, "FloatBorder", 1, 0, -1)
      ...
      for i = vis_start, vis_end do
        ...
        if i == st.sel then
          add_highlight(buf, "CursorLine", buf_line, 0, -1)
        end

        -- Star marker for saved theme
        if name == original then
          add_highlight(buf, "String", buf_line, 1, 2)
        end
        ...
        if entry_line then
          ...
          if vstart then
            ...
            add_highlight(buf, hl, buf_line, vstart - 1, vend)
          end
        end
      end
...
--- Switch to the last-used dark theme.
function M.dark()
  return theme.switch_to_dark()
end

--- Switch to the last-used light theme.
function M.light()
  return theme.switch_to_light()
end

--- Apply the custom txaty theme.
function M.txaty()
  return theme.apply "txaty"
end
